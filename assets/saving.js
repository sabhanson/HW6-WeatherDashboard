const searchBar = $('.searchBar').textContent
const submitBtn = $('.submitBtn');
let mainCardCity = $('.cityName');
let mainCardDate = $('.todayDate');
let mainCardTemp = $('.todayTemp');


//define all necessary variables


//today card only needs to be filled once, never appended like the five day cards


//URL for the WEATHER API call
var weatherAPI = `https://api.openweathermap.org/data/2.5/weather?q=${searchBar}&units=imperial&appid=d22455c31574a84b22d1c94f4c33f19c`;
//URL for the ONECALL API call
// var ONECALL = `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&units=imperial&appid=d22455c31574a84b22d1c94f4c33f19c`
...
function userInput () {
    ...
    .then(function (data) {
        //make some part of the data equal to the right part of the HTML doc
        var temp = data.main.temp;
        mainCardTemp.textContent = 'Temp: ' + temp + ' \xB0F'
    }
    )
}

//to create five day forecast, take the necessary fetch data and create/append a dayCard with the data in it. use the classes to apply the correct margins/spacing, etc.

// create new element (daycard)
//  - calendar Date
...
var searchInputValue = searchBar.value;
...
var cards = document.querySelectorAll('.dateTest')
...
function searchCity() {
    ...
    var weatherAPI = APIweather + 'vancouver' + '&units=imperial&appid=d22455c31574a84b22d1c94f4c33f19c';
    ...
    .then(function (response) {
      ...
      .then(function (data) {
        ...
        let timeUNIX = data.dt
        //put it through this moment method to return a nicely formatted MM/DD/YYYY day
        let testmoment = moment.unix(timeUNIX).format('L');
        console.log(testmoment + ' this is the UNIX number transformed into mm/dd/yyyy')
        todayDate.textContent = testmoment;
